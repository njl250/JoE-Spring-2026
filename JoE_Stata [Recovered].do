...
encode state, gen(state_id)
xtset state_id year
gen ln_solar = ln(solar_capacity_per_capita + 1)
gen ln_wind = ln(wind_capacity_per_capita + 1)
label var ln_solar "ln(Solar+1)"
label var ln_wind "ln(Wind+1)"

*=====================================================================
* GREEN BONDS -> SOLAR / WIND DEPLOYMENT
*=====================================================================

clear all
set more off
capture log close
log using "green_bonds_log.log", replace text
...
* State is a string (postal code) -> encode to numeric panel id
encode state, gen(state_id)
...
*=====================================================================
* PART A: SOLAR MODELS (1-8)
*=====================================================================


*-------------------------------------------------------------
* Model 1: ln(Solar) on gb
*-------------------------------------------------------------
estimates clear
...
*-------------------------------------------------------------
* Model 2: Add rps and GDP per cap
*-------------------------------------------------------------
regress ln_solar gb is_rps real_gdp_per_capita, vce(robust)
estimates store m2

*-------------------------------------------------------------
* Model 3: Add state and year FE (SEs clustered at state level)
*-------------------------------------------------------------
regress ln_solar gb is_rps real_gdp_per_capita i.state_id i.year, vce(cluster state_id)
estimates store m3

*-------------------------------------------------------------
* Model 4: Add gb x RPS interaction
*-------------------------------------------------------------
regress ln_solar i.gb##i.is_rps real_gdp_per_capita i.state_id i.year, ///
    vce(cluster state_id)
estimates store m4

*-------------------------------------------------------------
* Model 5: ln(Solar) on corporate and public dummies
*-------------------------------------------------------------
regress ln_solar is_corporate is_public, vce(robust)
estimates store m5

*-------------------------------------------------------------
* Model 6: Add rps and GDP per cap
*-------------------------------------------------------------
regress ln_solar is_corporate is_public is_rps real_gdp_per_capita, vce(robust)
estimates store m6

*-------------------------------------------------------------
* Model 7: Add state and year FE
*-------------------------------------------------------------
regress ln_solar is_corporate is_public is_rps real_gdp_per_capita ///
    i.state_id i.year, vce(cluster state_id)
estimates store m7

*-------------------------------------------------------------
* Model 8: Add gb x Corporate and gb x Public interactions
*-------------------------------------------------------------
regress ln_solar i.gb##i.is_corporate i.gb##i.is_public is_rps ///
    real_gdp_per_capita i.state_id i.year, vce(cluster state_id)
estimates store m8

*-------------------------------------------------------------
* SOLAR RESULTS TABLE (printed to console/log)
*-------------------------------------------------------------
estimates table m1 m2 m3 m4 m5 m6 m7 m8, ///
    b(%9.3f) star(0.10 0.05 0.01) ///
    stats(N r2) ///
    title("Table: Solar Deployment and Green Bonds")

* Also write the same table to a plain text file you can open/copy from
log using "solar_results.log", replace text name(solartab)
estimates table m1 m2 m3 m4 m5 m6 m7 m8, ///
    b(%9.3f) star(0.10 0.05 0.01) stats(N r2)
log close solartab

*=====================================================================
* PART B: WIND MODELS (9-16)
*=====================================================================


*-------------------------------------------------------------
* Model 9: ln(Wind) on gb
*-------------------------------------------------------------
regress ln_wind gb, vce(robust)
estimates store m9

*-------------------------------------------------------------
* Model 10: Add rps and GDP per cap
*-------------------------------------------------------------
regress ln_wind gb is_rps real_gdp_per_capita, vce(robust)
estimates store m10

*-------------------------------------------------------------
* Model 11: Add state and year FE
*-------------------------------------------------------------
regress ln_wind gb is_rps real_gdp_per_capita i.state_id i.year, vce(cluster state_id)
estimates store m11

*-------------------------------------------------------------
* Model 12: Add gb x RPS interaction
*-------------------------------------------------------------
regress ln_wind i.gb##i.is_rps real_gdp_per_capita i.state_id i.year, ///
    vce(cluster state_id)
estimates store m12

*-------------------------------------------------------------
* Model 13: ln(Wind) on corporate and public dummies
*-------------------------------------------------------------
regress ln_wind is_corporate is_public, vce(robust)
estimates store m13

*-------------------------------------------------------------
* Model 14: Add rps and GDP per cap
*-------------------------------------------------------------
regress ln_wind is_corporate is_public is_rps real_gdp_per_capita, vce(robust)
estimates store m14

*-------------------------------------------------------------
* Model 15: Add state and year FE
*-------------------------------------------------------------
regress ln_wind is_corporate is_public is_rps real_gdp_per_capita ///
    i.state_id i.year, vce(cluster state_id)
estimates store m15

*-------------------------------------------------------------
* Model 16: Add gb x Corporate and gb x Public interactions
*-------------------------------------------------------------
regress ln_wind i.gb##i.is_corporate i.gb##i.is_public is_rps ///
    real_gdp_per_capita i.state_id i.year, vce(cluster state_id)
estimates store m16
...
* WIND RESULTS TABLE
*-------------------------------------------------------------
estimates table m9 m10 m11 m12 m13 m14 m15 m16, ///
    b(%9.3f) star(0.10 0.05 0.01) ///
    stats(N r2) ///
    title("Table: Wind Deployment and Green Bonds")

log using "wind_results.log", replace text name(windtab)
estimates table m9 m10 m11 m12 m13 m14 m15 m16, ///
    b(%9.3f) star(0.10 0.05 0.01) stats(N r2)
log close windtab

*=====================================================================
* PART C: RESIDUAL DIAGNOSTICS & HETEROSKEDASTICITY CHECKS
*=====================================================================

* Solar: RPS interaction model
quietly regress ln_solar i.gb##i.is_rps real_gdp_per_capita i.state_id i.year
estimates store m4_ols

* Solar: Corporate/Public interaction model
quietly regress ln_solar i.gb##i.is_corporate i.gb##i.is_public is_rps ///
    real_gdp_per_capita i.state_id i.year
estimates store m8_ols

* Wind: RPS interaction model
quietly regress ln_wind i.gb##i.is_rps real_gdp_per_capita i.state_id i.year
estimates store m12_ols

* Wind: Corporate/Public interaction model
quietly regress ln_wind i.gb##i.is_corporate i.gb##i.is_public is_rps ///
    real_gdp_per_capita i.state_id i.year
estimates store m16_ols

foreach mod in m4_ols m8_ols m12_ols m16_ols {

    quietly estimates restore `mod'
    predict resid_`mod' if e(sample), residuals
    predict fitted_`mod' if e(sample), xb

    * Residuals vs fitted values
    twoway scatter resid_`mod' fitted_`mod', ///
        yline(0, lcolor(red)) ///
        title("Residuals vs Fitted: `mod'") ///
        ytitle("Residuals") xtitle("Fitted values") ///
        name(rvf_`mod', replace)
    graph export "rvf_`mod'.png", replace

    * Histogram of residuals
    histogram resid_`mod', normal ///
        title("Residual Distribution: `mod'") ///
        name(hist_`mod', replace)
    graph export "hist_`mod'.png", replace

    * Breusch-Pagan / Cook-Weisberg test
    display as text "----- Breusch-Pagan/Cook-Weisberg test: `mod' -----"
    estat hettest

    * White's test (may fail with many FE dummies - captured)
    capture estat imtest, white
    if _rc {
        display as error "White's test could not be computed for `mod' (likely too many parameters relative to N)."
    }
}
